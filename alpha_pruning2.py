...
def get_result(_metro, in_start, in_end, in_alpha):
    ...
    can_paths = find_all_paths(_metro.graph.cost_matrix, in_start, in_end)
    end_time = time.time()
    # print("all_paths : {} sec".format(end_time - start_time))
    for i in can_paths:
        can_paths2.append([cal_paths_cost(_metro, i), i])

    start_time = time.time()
    can_paths_alpha = find_all_paths_alpha(_metro.graph.cost_matrix, in_start, in_end, threshold)

    end_time = time.time()
    # print("all_paths_alpha : {} sec".format(end_time - start_time))
    return sorted(can_paths2, key=lambda cp: -cp[0])
